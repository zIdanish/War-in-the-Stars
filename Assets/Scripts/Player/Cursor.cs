...
public class Cursor : MonoBehaviour
{
    ...
    private void Start()
    {
        sprite = GetComponent<SpriteRenderer>(); // Init sprite variable
    }
    ...
    {
        MoveCursor();
        ShowCursor(!Clicked); // Disables the cursor when clicked
    }
    ...
    public void MoveCursor()
    {
        ...
        if (
            (delta.x != 0 || delta.y != 0) // Check if the mouse moved
            && (!Clicked) // Check if the mouse isnt clicked
        ){
    ...
    private void ShowCursor(bool on) // Toggles the cursor
    {
        sprite.enabled = on;
    }
}